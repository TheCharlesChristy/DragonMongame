...
socketio = SocketIO(app, cors_allowed_origins=["https://dragonmongame.com"], engineio_logger=True, logger=True)
#initialize socketio and allow cross origin requests from dragonmongame.com and enable logs
...
def Finaliseconnection(data):
    ...
    player.Sockid = sockid
    db.session.commit()
    characters = getactivecharacters(player.playerid)
    urls = generateUrls(characters)
    socketio.emit("Activecharacters", {"Characters":characters, "Urls":urls}, to = sockid)
...
def ClearMessages(Target):
    Sockid = request.sid
    check = Player.query.filter_by(username = Target)
    if check == None:
        type = "Group"
    else:
        type = "Friend"
    if type == "Friend":
        ...
        db.session.commit()
        listoftargetsockets =[]
        listofusernames = PlayerGroups.query.filter_by(Groupid = Target).all().username
        for i in range(len(listofusernames)):
            player = Player.query.filter_by(username = listoftargetsockets[i])
